...
Procedure execution state stays outside canonical OKF: `run prepare` creates an idempotent target-bound `ProcedureCapsule` with exact procedure hash, required bindings, principal, evidence and verification requirements; `run start --capsule` rechecks visibility, current state and hash before applying capsule task/principal defaults. `run start` creates an idempotent Markdown `ProcedureRun`; `run outcome` appends a hashed `ProcedureOutcome` record. Capsules, runs and outcomes are working state and never silently rewrite procedures.
