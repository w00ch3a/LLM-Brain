...
Experimental work is opt-in and separately flagged. `experiment status` reports prediction-error, goal-allocation, procedure-replay, causal-selection, learned-routing, latent-adapter, KV-adapter and multimodal-reconstruction gates. Only prediction records and procedure replay plans have local implementations, both disabled by default (`LLM_BRAIN_EXPERIMENT_PREDICTION_ERROR=1` and `LLM_BRAIN_EXPERIMENT_PROCEDURE_REPLAY=1`); the remaining capabilities record negative no-evidence gates and add no placeholder model, adapter or store.
