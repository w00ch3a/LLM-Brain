...
The `eval run` command writes a derived Markdown report and TSV trace under `evaluations/`. It compares raw source/episode retrieval with canonical lexical, configured vector, hybrid and graph paths and reports retrieval cost without mutating canonical memory. Evaluation identity binds the cases, canonical history, index state and configured executable hashes so changed evidence cannot reuse a stale report.
