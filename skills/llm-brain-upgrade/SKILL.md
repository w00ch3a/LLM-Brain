...
- The v0.6.1 package changes no storage schema and does not require vault migration; keep `migrate check` separate unless migration authority is explicit.
...
The apply command recomputes the plan and aborts on drift. It installs the hash-locked YAML runtime, uses the detected host's native update command, verifies the package and adapter without changing the v0.6.1 vault schema, restores changed package state on failure, and leaves sibling rollback trees plus the receipt. Any requested vault migration remains a separate, explicitly authorised operation.
