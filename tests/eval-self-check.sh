...
mkdir -p "$project_dir/okf/claims"
{
  ...
  printf '%s\n' 'generated: {by: test, at: 2026-08-07T00:00:00Z}' 'sources: [{resource: source://fixture}]'
...
first="$($cli --root "$vault" eval run "$project_id" --cases "$fixture/cases.tsv" --strategies none,source,episode,lexical,vector,hybrid,graph)"
output="$(printf '%s\n' "$first" | sed -n 's/.*output=\([^ ]*\).*/\1/p')"
[ -f "$output/report.md" ]
[ -f "$output/trace.tsv" ]
grep -Fq $'vector\tunsupported\ttrue' "$output/trace.tsv"
...
second="$($cli --root "$vault" eval run "$project_id" --cases "$fixture/cases.tsv" --strategies none,source,episode,lexical,vector,hybrid,graph)"
